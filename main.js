...
// V0.85 — Desktop Ready : toutes les données applicatives doivent vivre
// dans le dossier utilisateur Windows/macOS/Linux, pas dans le dossier du projet.
// database.js lit cette variable dès son chargement.
process.env.FOCUS_COMPTA_USER_DATA_DIR = app.getPath('userData');
process.env.FOCUS_COMPTA_APP_ROOT = __dirname;

const PDFParse = pdfParseModule.PDFParse;
const s3StorageV085 = require('./storage/s3-storage');
...
function sanitizeStorageSettingsForRendererV085(settings) {
    const cloned = JSON.parse(JSON.stringify(settings || {}));
    if (cloned.ovh && cloned.ovh.secretAccessKey) {
        cloned.ovh.secretAccessKey = '';
        cloned.ovh.secretKeySaved = true;
    }
    return cloned;
}

function buildOvhS3ConfigV085(settings) {
    const ovh = (settings || {}).ovh || {};
    return {
        endpoint: ovh.endpoint,
        region: ovh.region,
        bucket: ovh.bucket,
        accessKeyId: ovh.accessKeyId,
        secretAccessKey: ovh.secretAccessKey
    };
}
...
ipcMain.handle('get-desktop-storage-status-v085', async () => {
    const status = getDesktopStorageStatusV085();
    status.storage = sanitizeStorageSettingsForRendererV085(status.storage);
    return status;
});

ipcMain.handle('save-storage-settings-v085', async (event, data) => {
    const current = getStorageSettingsV085();
    const payload = data || {};
    if (payload.ovh && !payload.ovh.secretAccessKey && current.ovh && current.ovh.secretAccessKey) {
        payload.ovh.secretAccessKey = current.ovh.secretAccessKey;
    }
    const saved = saveStorageSettingsV085(payload);
    return sanitizeStorageSettingsForRendererV085(saved);
});

ipcMain.handle('test-ovh-s3-v085', async (event, data) => {
    const current = getStorageSettingsV085();
    const merged = {
        ...current,
        ...(data || {}),
    ...
    try {
        const result = await s3StorageV085.testConnection(buildOvhS3ConfigV085(merged));
...
ipcMain.handle('open-user-data-folder-v085', async () => {
    await shell.openPath(USER_DATA_ROOT);
    return true;
});

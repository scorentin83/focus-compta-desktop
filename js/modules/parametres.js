...
(function initTechnicalSettingsV043() {
    ...
    async function renderAliasesV043() {
        ...
        await refreshStorageStatusV085();
    ...
    async function refreshStorageStatusV085() {
        const box = document.getElementById('storageStatusV085');
        if (!box || !window.api.getDesktopStorageStatusV085) return;
        const status = await window.api.getDesktopStorageStatusV085();
        const storage = status.storage || {};
        const ovh = storage.ovh || {};
        const counts = status.counts || {};
        const mode = document.getElementById('storageModeV085');
        const endpoint = document.getElementById('ovhEndpointV085');
        const region = document.getElementById('ovhRegionV085');
        const bucket = document.getElementById('ovhBucketV085');
        const accessKey = document.getElementById('ovhAccessKeyV085');
        if (mode) mode.value = storage.mode || 'local';
        if (endpoint) endpoint.value = ovh.endpoint || '';
        if (region) region.value = ovh.region || 'gra';
        if (bucket) bucket.value = ovh.bucket || '';
        if (accessKey) accessKey.value = ovh.accessKeyId || '';
        box.innerHTML = `
            <div class="snapshot-grid-v043">
                <div><strong>${escapeHtmlV043(storage.mode || 'local')}</strong><span>Mode</span></div>
                <div><strong>${counts.documents || 0}</strong><span>Documents GED</span></div>
                <div><strong>${counts.statements || 0}</strong><span>Relevés</span></div>
                <div><strong>${counts.receipts || 0}</strong><span>Justificatifs banque</span></div>
                <div><strong>${counts.localDocuments || 0}</strong><span>Docs locaux</span></div>
                <div><strong>${counts.s3Documents || 0}</strong><span>Docs S3</span></div>
            </div>
            <p class="muted"><strong>Dossier données :</strong> ${escapeHtmlV043(status.dataDir || '')}</p>
            <p class="muted"><strong>Base SQLite :</strong> ${escapeHtmlV043(status.dbPath || '')}</p>
        `;
    }

    function readStorageFormV085() {
        return {
            mode: document.getElementById('storageModeV085')?.value || 'local',
            ovh: {
                endpoint: document.getElementById('ovhEndpointV085')?.value.trim() || '',
                region: document.getElementById('ovhRegionV085')?.value.trim() || 'gra',
                bucket: document.getElementById('ovhBucketV085')?.value.trim() || '',
                accessKeyId: document.getElementById('ovhAccessKeyV085')?.value.trim() || '',
                secretAccessKey: document.getElementById('ovhSecretKeyV085')?.value || ''
            }
        };
    }

    async function saveStorageSettingsV085() {
        if (!window.api.saveStorageSettingsV085) return;
        await window.api.saveStorageSettingsV085(readStorageFormV085());
        const secret = document.getElementById('ovhSecretKeyV085');
        if (secret) secret.value = '';
        await refreshStorageStatusV085();
        toastV043('Configuration stockage enregistrée.');
    }

    async function testOvhS3V085() {
        const resultBox = document.getElementById('storageTestResultV085');
        if (resultBox) resultBox.textContent = 'Test de connexion en cours…';
        const result = await window.api.testOvhS3V085(readStorageFormV085());
        if (resultBox) {
            resultBox.textContent = result && result.ok
                ? 'Connexion OVH S3 OK.'
                : `Connexion impossible : ${(result && result.message) || 'erreur inconnue'}`;
        }
        toastV043(result && result.ok ? 'Connexion OVH S3 OK.' : 'Connexion OVH S3 impossible.', result && result.ok ? 'success' : 'error');
    }
    ...
    function bindActionsV043() {
        const map = [
            ...
            ['saveStorageSettingsV085', saveStorageSettingsV085],
            ['testOvhS3V085', testOvhS3V085],
            ['openUserDataFolderV085', () => window.api.openUserDataFolderV085 && window.api.openUserDataFolderV085()]
        ];
    ...
    function refreshCurrentSettingsPanelV043(tab) {
        ...
        if (tab === 'storageV085') refreshStorageStatusV085();
    }
